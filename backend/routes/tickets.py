...
async def update_ticket(
    ...
    for field, value in update_data.items():
        ...
        if value != old_value:
            history = TicketHistory(
                ...
                changed_by="usuario",  # TODO: reemplazar por usuario real
            )
            session.add(history)
    ...
    try:
        ...
        logger.warning(
            "No se pudo regenerar el embedding en Redis para ticket editado %s: %s",
            ticket_id,
            e,
        )
...
async def delete_ticket(
    ...
    try:
        ...
        logger.warning(
            "No se pudo eliminar el embedding en Redis para %s: %s", redis_key, e
        )
